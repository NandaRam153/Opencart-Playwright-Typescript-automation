...
if [ -z "$TEST_USER_EMAIL" ] || [ -z "$TEST_USER_PASSWORD" ] || \
  ...
  echo "::error::Wishlist E2E requires TEST_USER_EMAIL and TEST_USER_PASSWORD when CI=true. Add repository secrets under Settings → Secrets and variables → Actions." >&2
  exit 1
fi
